...
def get_item_info(url):
    ...
    if soup.find('h1','title-name'):
        ...
        post_time = list(soup.select('.pr-5')[0].stripped_strings)
        type_info = soup.select('div.leftBox > div:nth-of-type(3) > div > ul > li:nth-of-type(1) > span > a')[0].text
        price = soup.select('.f22')[0].text
        trading_place = list(soup.select('div.leftBox > div:nth-of-type(3) > div > ul > li:nth-of-type(3)')[0].stripped_strings)
        data = {
            '商品标题':title,
            '发帖时间':post_time,
            '类型':type_info,
